...
** Anisotropic Flows and Background Curvature
...
** Proof
- Key Properties:
  - Time derivative is tangent: \(A(X) = D_t X = D_X (-f \nu - \grad_h f) = f\mathcal{W} (X) + \nabla_X \grad_h f\)
  - Fixed normal: \(\nabla_t \nu = 0\)
  - Fixed metric: \(\nabla_t g = 0\) \pause
- Affine fundamental form
  - \(\tilde{g}(X, Y) = \frac{1}{F} h(X, Y)\)
  - Define: \(B(X, Y) = \tilde{g}(A(X), Y)\) (not symmetric in non-flat spaces!)
  - Define: \(\Lambda(X, Y) = \Rm(\partial_t X_t, X, \nu, Y)\)
  - \(B(X, Y) - B(Y, X) = \frac{1}{f}\left(\Lambda(X, Y) - \Lambda(Y, X)\right)\) \pause
- Evolution
  - \(\nabla_t h = B + \Lambda\)
  - Remember \(\partial_t X_t\) is not normal so \(\Lambda\) is not sectional curvature!
** Proof
- Main Equation
  \[
  \partial_t u \geq \Box_{F} u + T \ast \nabla u + \frac{p+1}{p} u^2 + \text{Remainder} (\Rm, \nabla \Rm)
  \]
  where \(\Box_F u = dF(\nabla^2 u) = \dot{F}^{ij} \nabla^2_{ij} u\) \pause
